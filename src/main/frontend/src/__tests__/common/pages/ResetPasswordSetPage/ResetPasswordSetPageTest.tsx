...
import { ResetPasswordDto } from "../../../../features/registration/registrationApiSlice";

describe("ResendVerificationTokenPage", () => {
    const handlers = [
        rest.post("api/v1/resetPasswordSet", (req, res, ctx) => {
            return req.json().then((data: ResetPasswordDto) => {
                if (data.newPassword === "Password error") {
                ...
                if (data.newPassword === "Global error") {
                    const error: ErrorResponse = {
                ...
                if (data.newPassword === "Server error") {
                    const error: ErrorResponse = {
...
function fillResetInputs({ newPassword, matchingPassword }: ResetPasswordDto) {
    ...
    if (!newPasswordInput || !matchingPasswordInput) {
